require('dev/testing')
require('Codecs/Lua Codecs/blu/blu_control_remote')

command = {
  clearMidiQueue = function() MIDI_QUEUE = {} end
}

TestCluster:new('Context Utility Functions', {
  TestCase:new('val', {
  ...
  TestCase:new("err", {
    ...
    TestEntry:new('err.remoteMap', function()
      assertEqual('RemoteMapError: There is something wrong on the Remote Map File!', err.remoteMap())
    end),
  ...
  TestCase:new("num", {
    ...
    TestEntry:new('num.toHex', function()
      ...
      assertEqual('0', num.toHex(0, false))
      assertEqual('7', num.toHex(7, false))
      assertEqual('a', num.toHex(10, false))
      assertEqual('c6', num.toHex(198, false))
      assertEqual('fff', num.toHex(4095, false))
    end),
    TestEntry:new('num.round', function()
      assertEqual(1, num.round(1.2))
      assertEqual(2, num.round(1.5))
      assertEqual(2, num.round(1.7))

      assertEqual(1.1, num.round(1.12, 1))
      assertEqual(1.2, num.round(1.15, 1))
      assertEqual(1.2, num.round(1.17, 1))

      assertEqual(1.41, num.round(1.412, 2))
      assertEqual(1.42, num.round(1.415, 2))
      assertEqual(1.42, num.round(1.417, 2))
    end)
  }),
  TestCase:new("str", {
    ...
    TestEntry:new('str.pad', function()
      assertEqual('test', str.pad('test', 2))
      assertEqual('test   ', str.pad('test', 7))
      assertEqual('test---', str.pad('test', 7, { char = '-' }))
      assertEqual('   test', str.pad('test', 7, { leading = true }))
    end),
    TestEntry:new('str.strip', function()
      assertEqual('salami', str.strip('salami'))
      assertEqual('salami', str.strip('    salami    '))
      assertEqual('salami sauce', str.strip('    salami sauce     '))
    end),
  ...
  TestCase:new("tbl", {
    ...
    TestEntry:new('tbl.shallowCopy', function()
      ...
      assertTrue(tostring(original.deep) == tostring(copy.deep))
    end),
  ...
  TestCase:new("dct", {
    ...
    TestEntry:new('dct.get', function()
      local t = { a = 3, b = 4, f = false }
      assertEqual(3, dct.get(t, 'a'))
      assertTableEqual({ a = 3, b = 4, f = false }, t)
  ...
  TestCase:new("lst", {
    ...
    TestEntry:new('lst.concat', function()
      assertTableEqual({}, lst.concat())
      assertTableEqual({ 'a', 'b' }, lst.concat({ 'a', 'b' }))
      assertTableEqual({ 'a', 'b', 'c', 'd' }, lst.concat({ 'a', 'b' }, { 'c', 'd' }))
      assertTableEqual({ 'a', 'b', 'c', 'd', 'e', 'f', 'g' }, lst.concat({ 'a', 'b' }, { 'c', 'd' }, { 'e', 'f', 'g' }))
    end),
...
TestCluster:new('General Utility Functions', {
  TestCase:new('fif', {
    TestEntry:new('"(condition) and false or true" sucks...', function()
      -- the wannabe-tertiary operator of Lua can never evaluate the second operand to false.
      -- that behaviour makes it kinda dangerous...
      assertEqual(true, true and true or false)
      assertEqual(false, false and true or false)
      assertEqual(true, true and false or true) -- this is sad. FU Lua!
      assertEqual(true, false and false or true)
    end),
    TestEntry:new('fif returns correct boolean depending on condition', function()
      assertEqual(true, fif(true, true, false))
      assertEqual(false, fif(false, true, false))
      assertEqual(false, fif(true, false, true)) -- CORRECT!!
      assertEqual(true, fif(false, false, true))
    end),
    TestEntry:new('fif returns correct string depending on condition', function()
      assertEqual('yes', fif(true, 'yes', 'no'))
      assertEqual('no', fif(false, 'yes', 'no'))
      assertEqual('no', fif(true, 'no', 'yes'))
      assertEqual('yes', fif(false, 'no', 'yes'))
    end),
  }),
}, true)

TestCluster:new('Midi Functions', {
  TestCase:new("midi", {
    TestEntry:new('midi.pattern', function()
      ...
      assertEqual('800000', midi.pattern(midi.ne, 0, 0))
      assertEqual('c04b7c', midi.pattern(midi.pc, 75, 124))
      assertEqual('c0750x', midi.pattern(midi.pc, '75', '0x'))
    end),
    TestEntry:new('midi.addToQueue', function()
      assertError(function() midi.addToQueue('901020') end, 'TypeError: Expected a value of type table, but got string.')

      command.clearMidiQueue()
      midi.addToQueue({ midi.cc, 0, 0 })
      assertTableEqual({ { midi.cc, 0, 0 } }, MIDI_QUEUE)
      midi.addToQueue({ midi.ss, 0, 0, 0, midi.se })
      assertTableEqual({ { midi.cc, 0, 0 }, { midi.ss, 0, 0, 0, midi.se } }, MIDI_QUEUE)
      command.clearMidiQueue()
    end)
  }),
}, true)
