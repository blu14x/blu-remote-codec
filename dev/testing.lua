...
assertTableEqual  = function(expected, actual, _el)
  _el           = _el or 2
  local isEqual = true
  for k, v in pairs(expected) do
    if type(v) == 'table' then
      if not assertTableEqual(v, actual[k], _el + 1) then
        isEqual = false
        break
      end
  ...
  for k, v in pairs(actual) do
    if type(v) == 'table' then
      if not assertTableEqual(v, expected[k], _el + 1) then
        isEqual = false
        break
      end
  ...
  if not isEqual then error('Expected & actual tables are not equal', _el) end
  return isEqual
end
